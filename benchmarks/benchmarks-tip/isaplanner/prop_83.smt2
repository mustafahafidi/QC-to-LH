...
(declare-sort sk_a 0)
(declare-datatypes ()
  ((list3 (nil3) (cons3 (head3 sk_a) (tail3 list3)))))
(declare-datatypes ()
...
(declare-fun zip (list3 list2) list)
...
(assert
  (not
    (forall ((xs list3) (ys list3) (zs list2))
      (= (zip (append2 xs ys) zs)
        (append (zip xs (take (len xs) zs))
          (zip ys (drop (len xs) zs)))))))
(assert
  (forall ((x list3) (y list2))
    (= (zip x y)
      (ite
        (is-cons3 x)
        (ite
          (is-cons2 y)
          (cons (Pair2 (head3 x) (head2 y)) (zip (tail3 x) (tail2 y))) nil)
        nil))))
(assert
