...
prop_Merge hl@(Node x h11 h12) hr@(Node y h21 h22) 
  ...
  | otherwise = ()
                ? req_order hr hl
                ? Test_Heap.prop_Merge_subProof hr hl
...
prop_Merge_subProof hl@(Node x h11 h12) hr@(Node y h21 h22) = 
                        ()
                        ? invariant (hl `merge` hr)
                        -- ? (hl `merge` hr)
                        ? (req_order hl hr)
